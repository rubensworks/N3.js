...
N3Parser.prototype = {
  ...
  // ### `_readContext` reads a quad's optional context.
  _readContext: function (token) {
    switch (token.type) {
    case 'IRI':
      if (this._baseURI === null || absoluteURI.test(token.value))
        this._context = token.value;
      else
        this._context = this._resolveURI(token.value);
      break;
    case 'prefixed':
      if (token.prefix === '_') {
        this._context = this._blankNodes[token.value] ||
          (this._blankNodes[token.value] = '_:b' + blankNodeCount++);
      }
      else {
        var prefix = this._prefixes[token.prefix];
        if (prefix === undefined)
          return this._error('Undefined prefix "' + token.prefix + ':"', token);
        this._context = prefix + token.value;
      }
      break;
    case 'dot':
    case 'semicolon':
    case 'comma':
      ...
      return this._readPunctuation(token, false);
    default:
      return this._error('Expected punctuation or context to follow "' + this._object + '"', token);
    }
  ...
  _readBlankNodeTail: function (token, empty) {
    ...
    if (empty !== true)
      this._callback(null, { subject: this._subject,
                             ...
                             object: this._object,
                             context: this._context || 'n3/contexts#default' });
  ...
  _readListItem: function (token) {
    ...
    case 'listend':
      ...
      // The list was in the parent triple's object.
        ...
        this._context = parentTriple.context;
